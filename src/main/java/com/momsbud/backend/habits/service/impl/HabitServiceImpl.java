...
public class HabitServiceImpl implements HabitService {
    ...
    public HabitResponse create(String userId, CreateHabitRequest req) {
        ...
        if (h.getFrequency() == HabitFrequency.WEEKLY) {
            ...
            if (dows.isEmpty()) throw new IllegalArgumentException("daysOfWeek required for WEEKLY");
    ...
    public void log(String userId, String habitId, LogHabitRequest req) {
        ...
        var existing = logRepo.findByHabitIdAndUserIdAndLogDateAndIsDeletedFalse(h.getId(), userId, today).orElse(null);
        if (existing != null) {
    ...
    public List<TodayHabitItem> today(String userId) {
        List<Habit> habits = habitRepo.findAllByUserIdAndIsDeletedFalseOrderByCreatedAtDesc(userId)
                .stream().filter(Habit::isActive).collect(Collectors.toList());
        ...
        return habits.stream()
                .filter(h -> {
                    ...
                    if (h.getFrequency() == HabitFrequency.DAILY) return true;
                    Integer[] arr = h.getDaysOfWeek();
                    if (arr == null || arr.length == 0) return false;
                    for (Integer i : arr) if (i != null && i == dow) return true;
                    return false;
                })
